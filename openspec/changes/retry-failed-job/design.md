...
- [glab changes the `Retried job (ID: …)` wording] → the parse is confined to one exported function with its own unit test, and a parse miss degrades to the documented graph fallback rather than a wrong behaviour. The fixture records the format string found in the installed glab 1.117.0 binary; the operator's live restarts then confirmed the behaviour end to end (the log screen followed the id parsed from stdout, so a parse miss would have sent it back to the graph instead). The one thing not captured is the byte-exact live stdout line, including any decoration glab adds on a TTY; the parser strips SGR to cover that.

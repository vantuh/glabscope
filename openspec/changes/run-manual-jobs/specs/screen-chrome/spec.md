...
- **THEN** confirming starts nothing in GitLab and shows a short non-fatal message saying why: the action's refusal wording, that the job's status changed, or that the job is no longer in this pipeline
...
- **THEN** no prompt opens and no GitLab command is started, and once the refresh settles the same key asks normally

...
`pendingJobAction(model)` returns the job the open prompt would act on, looked up by job id in the current graph and re-gated with `jobActionKind`. The reducer's confirm step resolves the prompt through that same helper, and the key handler calls it to decide whether to spawn a process at all, so the re-check exists once. **A confirmation waits for a graph answer, not for a flag.** While a graph refresh this operator asked for is in flight (`manualRefresh === "graph"`), `confirmJobAction` marks the open prompt `waiting` instead of resolving it, and the prompt says so ("waiting for the refresh…  esc cancel"). The wait ends on the next **graph** answer — `refreshGraph`, or `refreshError` carrying `source: "graph"` — and the model counts graph answers so an answer to something else (the pipeline list's own refresh, succeeding or failing) can never release it. Repeated Enter while waiting changes nothing, so a second key cannot short-circuit the wait either.

Confirming during an unanswered refresh is exactly the case where the operator has already told the app that newer data is coming, and for a `play` the stale answer is the harmful one: GitLab's invalid-transition fallback would spawn a second attempt for a manual job that someone had already enqueued. Escape still cancels the wait, and a failed *graph* refresh ends it against the graph that is still on screen, because no newer answer exists.

The re-check also requires the job's action to still be **the action the operator confirmed**: `jobActionKind` must return the prompt's own kind, so a failed job that turned manual cannot answer a confirmed "retry" with a play, and a manual job that turned canceled cannot answer a confirmed "run" with a retry.

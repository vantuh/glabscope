...
The wait is not a trap: the graph keeps polling while it is on screen, so a newer answer always arrives; Escape cancels the wait, and leaving the screen clears the prompt. On the log screen no polling runs, so a *failed* manual refresh there leaves the prompt waiting until the operator cancels it or goes back — one key either way, and nothing is started meanwhile.

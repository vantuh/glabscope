...
`pendingJobAction(model)` returns the job the open prompt would act on, looked up by job id in the current graph and re-gated with `jobActionKind`. The reducer's confirm step resolves the prompt through that same helper, and the key handler calls it to decide whether to spawn a process at all, so the re-check exists once. **A confirmation waits for a refresh the operator asked for.** While `manualRefresh` is set, `confirmJobAction` marks the open prompt `waiting` instead of resolving it, and the prompt says so ("waiting for the refresh…  esc cancel"); `refreshGraph` and `refreshError` settle it the moment that answer (or its failure) lands. Confirming during an unanswered refresh is exactly the case where the operator has already told the app that newer data is coming, and for a `play` the stale answer is the harmful one: GitLab's invalid-transition fallback would spawn a second attempt for a manual job that someone had already enqueued. Escape still cancels the wait, and a failed refresh settles against the graph that is still on screen, because no newer answer exists.

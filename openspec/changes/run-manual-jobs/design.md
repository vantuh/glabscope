...
The same-frame latch is set exactly when the gate allows the action, so it cannot swallow a key for a prompt that will not open. The one residual frame: if a refresh commits between the key press and the reducer's answer and the job no longer qualifies, the reducer refuses, shows its notice, and only that frame's remaining keys are ignored — the next key acts normally.

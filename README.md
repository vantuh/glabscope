...
Keys: `q` quits, arrows move (up/down in a stage, left/right across stages), Enter drills in (job attempts when a card was retried, otherwise the log), `Ctrl+R` retries the focused failed or canceled job or runs a waiting manual job, after confirming with Enter, Esc goes back (log → attempts if open → graph → list).
